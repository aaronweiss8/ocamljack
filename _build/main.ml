  ...
  | h::t -> ANSITerminal.(print_string [blue]
    ...
    if Player.is_user h then
      ...
      if Chip.get_value to_bet < min_bet then 
        ...
        ((Chip.create_bot_bet min_bet (Player.chips h))::accum)
...
and double_down game ind on_dealer =
  ...
  with
  | Chip.Not_Within -> ANSITerminal.(
      print_string [red] ("\nYou cannot double down, you need more chips\n"));
  ...
  if List.length (Blackjack.get_players game) = 0 then
    ...
    let do_beginning game =
      ...
      | h::t ->
        ...
        (*USER HAS THE OPTION TO BREAK OR SIMPLIFY THEIR CHIPS*)
        ...
        let bet_game = Blackjack.place_initial_bets simp_break_ed_game bets in
        (*AT THIS POINT THE USER HAS PLACED THEIR INITIAL BET *)
        Blackjack.get_info bet_game true;
        ANSITerminal.(print_string [red;Bold] ("\n========================\n"^
                                               "Cards have been dealt" ^
                                               "\n========================\n")); 
        (Blackjack.deal_initial_cards bet_game) in
