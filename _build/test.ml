...
(**TEST PLAN
   ...
   - Player creation, updating the player type to account for changes in bets,
     ...
     Blackjack:
   - Main user actions: hit, split, stand, and double down
   ...
   - Stress testing the amount of bots that the program can handle
     ...
     If you would like to test without
     playing, an easy way is to run through the program with 1000 bots and see
...
let added_to_deck = empty_deck |> Cards.add_to_deck black_ace_hearts
...
(* Example Players *)
let player1 = Player.new_player "Aaron" added_chips [added_to_deck] 
    [sblue_chip] false
...
let p2_hand = empty_deck |> Cards.add_to_deck red_10_hearts |>
              Cards.add_to_deck black_ace_hearts 
...
let player2 = Player.new_player "Covid" test_chip [p2_hand] 
    [sblue_chip] false
let player3 = Player.new_player "Bryan" test_chip_2 [added_to_deck] 
    [sblue_chip] false
let playerlist = [player1;player2;player3]
let playerlist2 = [player1_split;player2;player3]
let playerlist3 = [player2;player3]
let playerlist_with_bets = [(Player.new_player "asdf" test_chip
                               [] [bred_chip] false);
                            (Player.new_player "asdf" test_chip 
                               [] [bred_chip] false)]
...
let dealer_bad = Player.new_player "dealer" Chip.empty 
    [[Cards.make_card Heart Red (Num 2);Cards.make_card 
        Heart Red (Num 2)]] [Chip.empty] true
let dealer_bj = Player.new_player "dealer" Chip.empty 
    [[Cards.make_card Heart Red Ace;Cards.make_card Heart Red King]] 
    [Chip.empty] true
let chip_tests = "Chip tests" >:::[
    ...
    "get_value of valued chip" >:: 
    (fun _-> assert_equal (1 + 10 + 50 + 150 + 200) 
        (get_value test_chip));

    "makes sure simplify preserves value" >:: 
    (fun _-> assert_equal 
        (1 + 10 + 50 + 150 + 200)
        (get_value  (Chip.simplify test_chip 1)));

    "makes sure simplify preserves value (white)" >:: 
    (fun _-> assert_equal 
        5 (get_value  (Chip.simplify swhite_chip 1)));

    "makes sure simplify preserves value (red)" >:: 
    (fun _-> assert_equal 
        10 (get_value  (Chip.simplify sred_chip 1)));

    "makes sure simplify preserves value (blue)" >::
    (fun _-> assert_equal 
        25 (get_value  (Chip.simplify sblue_chip 1)));

    "makes sure simplify preserves value (green)" >:: 
    (fun _-> assert_equal 
        100 (get_value  (Chip.simplify sgreen_chip 1)));

    "makes sure break preserves value (red)" >::
    (fun _-> assert_equal 
        5 (get_value (Chip.break bred_chip 1)));

    "makes sure break preserves value (blue)" >:: 
    (fun _-> assert_equal 
        10 (get_value  (Chip.break bblue_chip 1)));

    "makes sure break preserves value (green)" >:: 
    (fun _-> assert_equal 
        25 (get_value  (Chip.break bgreen_chip 1)));

    "makes sure break preserves value (black)" >:: 
    (fun _-> assert_equal 100 (get_value  (Chip.break bblack_chip 1)));
...
let deck_tests = "Deck tests" >::: [
    ...
    "standard deck, compare, shuffle " >:: (fun _-> 
        assert_equal true (List.fold_right (fun x y -> x && y ) 
                             (standard_deck |> shuffle |> List.map
                                (fun x -> List.mem x standard_deck)) true));
    "add to deck, remove inst" >:: (fun _-> assert_equal 
                                       (empty |> add_to_deck black_ace_hearts)
                                       (remove_single_instance black_ace_hearts
                                          added_to_deck));
    ...
    "transfer card" >:: (fun _-> assert_equal 
                            ((empty |> add_to_deck black_ace_hearts),
                             (empty |> add_to_deck black_ace_hearts))
                            (transfer_card added_to_deck 
                               empty black_ace_hearts));
    "get rank" >:: (fun _-> assert_equal (Num 11) (get_rank red_11_hearts));
    "get rank" >:: (fun _-> assert_equal (Jack)
                       (get_rank (make_card Heart Red Jack)));
...
let player_tests = "Player tests" >::: [
    ...
    "player get hand test" >::
    (fun _ -> assert_equal [added_to_deck] (Player.get_hand player1));

    "player get chips test" >::
    (fun _ -> assert_equal added_chips (Player.chips player1));

    "player chips value test" >:: (fun _ -> assert_equal 996 
                                      (Player.chips_value player1));

    "player bet test" >:: (fun _ -> assert_equal [sblue_chip] 
                              (Player.bet player1));

    "player bet value test" >:: (fun _ -> assert_equal 25 
                                    (Player.bet_value player1 0));

    "player add chips test" >:: (fun _ -> 
        assert_equal 1021 (Player.chips_value 
                             (Player.add_chips sblue_chip player1)) );

    "player bet chips test" >:: (fun _ -> 
        assert_equal 436 (Player.bet_value 
                            (Player.bet_chips test_chip 0 player1) 0 ));
    ...
    "player add to hand test" >:: (fun _ -> 
        assert_equal [black_ace_hearts::added_to_deck] 
          (Player.get_hand 
             (Player.add_to_hand black_ace_hearts 0 player1)));

    "player add bet test" >:: 
    (fun _ -> assert_equal 2 (List.length(Player.bet(Player.add_bet player1))));
...
let blackjack_tests = "Blackjack tests" >::: [
    ...
    "test hit" >:: (fun _-> 
        assert_equal (hit game1 0 false |> current_player |> Player.get_hand 
                      |> List.hd) 
    ...
    "test double_down" >:: (fun _-> assert_equal 50
                               (double_down game1 0 |> current_player |> 
                                Player.bet |> List.hd |> Chip.get_value));

    "test check_if_soft, not soft" >:: (fun _-> assert_equal false 
                                           (Cards.check_if_soft not_soft));

    "test check_if_soft, soft" >:: (fun _-> assert_equal true 
                                       (Cards.check_if_soft soft));

    "test check_if_soft, soft_2" >:: (fun _-> assert_equal true 
                                         (Cards.check_if_soft soft_2));

    "hard 12" >:: (fun _-> assert_equal false 
                      (Cards.check_if_soft hard12));
